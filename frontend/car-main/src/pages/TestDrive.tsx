...
export const TestDrive = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // car-main пока хранит машины/дилеров локально, поэтому отправляем как comment.
      // Когда перейдём на реальные id из backend, можно будет передавать carId/dealerId.
      const commentParts = [
        formData.comment ? `Комментарий: ${formData.comment}` : '',
        formData.car ? `Авто: ${formData.car}` : '',
        formData.dealer ? `Дилер: ${formData.dealer}` : ''
      ].filter(Boolean);
      ...
      setIsSubmitted(true);
      setFormData({ name: '', phone: '', car: '', dealer: '', comment: '' });
          ...
          {isSubmitted ? (
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {error ? (
                  <p className="text-[11px] text-red-400/90 text-center tracking-[0.15em]">
                    {error}
                  </p>
                ) : null}
