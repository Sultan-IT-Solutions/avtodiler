...
export const Catalog = () => {
  ...
  const { data: cars, loading, error, reload } = useCatalog();
  const safeCars = cars ?? [];
  ...
  const brands = useMemo(() => {
    const list = safeCars
      .map((c: any) => c?.make?.nameRu || c?.make?.nameEn || c?.make?.nameKz || c?.brand)
      .filter(Boolean) as string[];
    return Array.from(new Set(list)).sort();
  }, [safeCars]);

  const filteredCars = useMemo(() => {
    let filtered = safeCars.filter((car: any) => {
      const brandValue = car?.make?.nameRu || car?.make?.nameEn || car?.make?.nameKz || car?.brand;
      const brandMatch =
        selectedBrands.length === 0 || (brandValue ? selectedBrands.includes(String(brandValue)) : false);
    ...
    switch (sortBy) {
      case 'newest':
        filtered.sort((a: any, b: any) => (Number(b.year) || 0) - (Number(a.year) || 0));
        break;
      case 'priceHigh':
        filtered.sort((a: any, b: any) => (Number(b.price) || 0) - (Number(a.price) || 0));
        break;
      case 'priceLow':
        filtered.sort((a: any, b: any) => (Number(a.price) || 0) - (Number(b.price) || 0));
        break;
    }
    return filtered;
  }, [safeCars, selectedBrands, yearRange, priceRange, sortBy]);
      ...
      {error && (
        <div className="bg-red-950/40 border-b border-red-500/20 text-red-100">
          <div className="container mx-auto px-6 lg:px-16 py-3 flex items-center justify-between gap-4">
            <div className="text-sm">Ошибка загрузки каталога: {error}</div>
        ...
        >
          ...
          <div className="hero-meta mt-10 opacity-0 flex items-center gap-3">
            <span
              ...
              style={MONO_FONT}
            >
              {loading ? '--' : String(filteredCars.length).padStart(2, '0')}
            </span>
